...
export default {
  ...
  computed: {
    ...
    percent: function () {
      console.log(this.wrongLen)
      console.log(this.wordsLen)
      console.log(this.words)
      return 100 - (this.wrongLen / this.wordsLen).toFixed(2) * 100
    }
  },
  methods: {
  ...
  created () {
    this.$store.commit('setTestIndex', 0)
    let wrong = this.$store.state.word.testWrong
    let words = this.$store.state.word.words
    ...
    this.$ajax({
      ...
      data: {
        ...
        dayNum: this.wordsLen,
        status: 1,
        wrongNum: this.wrongLen
      }
    })
    .then((res) => {
      if(res.data >= 0) {
